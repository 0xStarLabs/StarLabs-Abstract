...
export class AbstractAccount {
        ...
        try {
            if (this.privyPrivateKey === "") {
                ...
                if (this.privyPrivateKey === "") {
                    ...
                    logger.success(
                        `${this.address} | Privy private key saved - ${this.privyPrivateKey}. Waiting for 30 seconds after the registration...`
                    );
                    await sleep(30000);
                }
            }
    ...
    async collectAllToEth(): Promise<boolean> {
        return await this.retryOperation(async () => {
            try {
                ...
                if (!balances || balances.length === 0) {
                    logger.info(
                        `${this.accountIndex} | ${this.address} | No tokens to collect to ETH`
                    );
                    return true;
                }
                ...
                logger.info(
                    `${this.accountIndex} | ${
                        this.address
                    } | Current balances: ${this.formatBalances(balances)}`
                );

                // Filter out ETH and tokens with zero balance
                const tokensToSwap = balances.filter(
                    (b) =>
                        b.symbol !== "ETH" &&
                        b.symbol !== "WETH" &&
                ...
                if (tokensToSwap.length === 0) {
                    logger.info(
                        `${this.accountIndex} | ${this.address} | No tokens to swap to ETH`
                    );
                    return true;
                }
                ...
                for (const token of tokensToSwap) {
                    ...
                    for (let attempt = 0; attempt < 3 && !success; attempt++) {
                        try {
                            ...
                            const mappedSymbol =
                                TOKEN_SYMBOL_MAPPING[token.symbol] ||
                                token.symbol;
                            ...
                            if (!originTokenAddress) {
                                logger.error(
                                    `${this.accountIndex} | ${this.address} | Could not find address for token ${token.symbol} (mapped: ${mappedSymbol})`
                                );
                                continue;
                            }
                            ...
                            logger.info(
                                `${this.accountIndex} | ${
                                    this.address
                                } | Swapping ${swapPercent}% (${ethers.formatUnits(
                                    amountToSwap,
                                    token.decimals
                                )} ${token.symbol}) to ETH`
                            );
                            ...
                            if (txHash) {
                                success = true;
                                logger.success(
                                    `${this.accountIndex} | ${this.address} | Successfully swapped ${token.symbol} to ETH: ${EXPLORER_URL}/tx/${txHash}`
                                );
                                ...
                                if (updatedBalances) {
                                    logger.info(
                                        `${this.accountIndex} | ${
                                            this.address
                                        } | Current balances: ${this.formatBalances(
                                            updatedBalances
                                        )}`
                                    );
                                }
                            ...
                            if (attempt < 2) {
                                ...
                                logger.error(
                                    `${this.accountIndex} | ${this.address} | All swap attempts failed for ${token.symbol}. Error: ${error}`
                                );
                            }
                        }
                    }
                ...
                if (finalBalances) {
                    logger.info(
                        `${this.accountIndex} | ${
                            this.address
                        } | Final balances after collecting to ETH: ${this.formatBalances(
                            finalBalances
                        )}`
                    );
                }
                ...
                logger.error(
                    `${this.accountIndex} | ${this.address} | Error collect all to eth: ${error}`
                );
                return false;
            }
        }, "Collect all to eth");
    ...
    async swaps(): Promise<boolean> {
        return await this.retryOperation(async () => {
            try {
                ...
                logger.info(
                    `${this.accountIndex} | ${this.address} | Will try to perform ${numberOfSwaps} swaps`
                );
                        ...
                        try {
                            ...
                            if (
                                ...
                                if (arbitrumBalance < minRequired) {
                                    if (
                                        !this.config.withdraw.withdraw_enabled
                                    ) {
                                        logger.info(
                                            `${this.accountIndex} | ${this.address} | Withdrawals are disabled in config, skipping...`
                                        );
                                        continue;
                                    }
                                    ...
                                    logger.info(
                                        `${this.accountIndex} | ${this.address} | Arbitrum balance is sufficient, proceeding with bridge...`
                                    );

                                    // Calculate safe amount to bridge (95% of balance or max from config, whichever is lower)
                                    const maxFromConfig = ethers.parseEther(
                                        this.config.bridge.eth_to_bridge[1].toString()
                                    );
                                    const safeBalance =
                                        (arbitrumBalance * BigInt(95)) /
                                        BigInt(100);
                                    const maxToUse =
                                        safeBalance < maxFromConfig
                                            ? safeBalance
                                            : maxFromConfig;
                                    ...
                                    logger.info(
                                        `${this.accountIndex} | ${
                                            this.address
                                        } | Will bridge ${ethers.formatEther(
                                            randomAmount
                                        )} ETH`
                                    );
                                    const ok = await this._executeBridge(
                                        randomAmount
                                    );
                                    if (!ok) {
                                        continue;
    ...
    public async votes(): Promise<boolean> {
        try {
            const numberOfVotes = randomInt(
                this.config.abs.number_of_votes[0],
                this.config.abs.number_of_votes[1] + 1
            );

            logger.info(
                `${this.accountIndex} | ${this.address} | Will perform ${numberOfVotes} votes`
            );
                ...
                const success = await this.retryOperation(async () => {
                    try {
                        ...
                        if (unvotedApps.length === 0) {
                            logger.info(
                                `${this.accountIndex} | ${this.address} | No more apps to vote for`
                            );
                            return true;
                        }

                        const randomApp = await randomItemFromArray(
                            unvotedApps
                        );

                        logger.info(
                            `${this.accountIndex} | ${this.address} | Vote ${
                                voteCount + 1
                            }/${numberOfVotes} | Voting for ${
                                randomApp.name
                            } | ID: ${randomApp.id}`
                        );
                        ...
                        if (txHash) {
                            logger.success(
                                `${this.accountIndex} | ${
                                    ...
                                    voteCount + 1
                                }/${numberOfVotes} successful for ${
                                    randomApp.name
                                } | TX: ${EXPLORER_URL}${txHash}`
                            );
                            ...
                            if (voteCount < numberOfVotes - 1) {
                                const pauseTime = randomInt(
                                    this.config.settings
                                        .random_pause_between_actions[0],
                                    this.config.settings
                                        .random_pause_between_actions[1]
                                );
                                logger.info(
                                    `${this.accountIndex} | ${this.address} | Waiting ${pauseTime} seconds before next vote...`
                                );
                                await sleep(pauseTime * 1000);
                            }
                            return true;
                        ...
                        logger.error(
                            `${this.accountIndex} | ${this.address} | Error voting for app: ${error}`
                        );
                        return false;
                    }
    ...
    async collectAllData(): Promise<boolean> {
        return await this.retryOperation(async () => {
            try {
                ...
                logger.error(
                    `${this.address} | Error collect all data: ${error}`
                );
                return false;
            }
        }, "Collect all data");
    ...
    async _bridge(): Promise<boolean> {
        try {
            ...
            if (arbitrumBalance < minRequired) {
                ...
                logger.info(
                    `${this.accountIndex} | ${this.address} | Withdrawing...`
                );
                ...
                const pauseTime = randomInt(
                    this.config.settings.pause_after_withdrawal[0],
                    this.config.settings.pause_after_withdrawal[1]
                );
                logger.info(
                    `${this.accountIndex} | ${
                        this.wallet.address
                    } | Waiting ${pauseTime.toString()} seconds after withdrawal`
                );
                await sleep(pauseTime * 1000);
                ...
                const maxToUse =
                    safeBalance < maxFromConfig ? safeBalance : maxFromConfig;
                ...
                const randomAmount =
                    minEth +
                    BigInt(
                        Math.floor(Math.random() * Number(maxToUse - minEth))
                    );

                logger.info(
                    `${this.accountIndex} | ${
                        this.address
                    } | Will bridge ${ethers.formatEther(randomAmount)} ETH`
                );
        ...
        try {
            ...
            if (!responseData.ok) {
                if (
                    ...
                    logger.error(
                        `${this.accountIndex} | ${this.address} | Twitter is rate limited. Please try again later.`
                    );
                    return false;
                }
                throw new Error(
            ...
            if (url.includes("www.abs.xyz/rewards?twitterHandle=")) {
                const username = url.split(
                    "www.abs.xyz/rewards?twitterHandle="
                )[1];
                logger.success(
                    `${this.accountIndex} | ${this.address} | Successfully connected twitter ${username}.`
                );
                return true;
            } else if (url.includes("error=duplicate")) {
                logger.error(
                    `${this.accountIndex} | ${this.address} | This twitter token is already connected to another account.`
                );
                return false;
            }
