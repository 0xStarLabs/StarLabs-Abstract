...
export class PrivateKeyGrabber {
    ...
    async grabPrivateKey(): Promise<string> {
        for (
            ...
            try {
                logger.info(
                    `${this.accountIndex} | Attempt ${attempt}/${this.config.settings.attempts} to grab private key`
                );
                ...
                if (!this.metamaskLoginPage) {
                    logger.error(
                        `${this.accountIndex} | MetaMask page not found, retrying...`
                    );
                    continue;
                }
                ...
                logger.error(
                    `${this.accountIndex} | Error grabbing private key (attempt ${attempt}/${this.config.settings.attempts}): ${error}`
                );
            } finally {
                await this.cleanup();
            }
            ...
            if (attempt < this.config.settings.attempts) {
                ...
                logger.info(
                    `${this.accountIndex} | Waiting ${pauseTime} seconds before next attempt...`
                );
        ...
        logger.error(
            `${this.accountIndex} | Failed to grab private key after ${this.config.settings.attempts} attempts`
        );
        return "";
    }
    ...
    async _loginAbs(): Promise<boolean> {
        for (let attempt = 1; attempt <= this.config.settings.attempts; attempt++) {
            try {
                ...
                this.metamaskLoginPage = this.context?.pages()[2];
                await this.metamaskLoginPage?.click(
                    'xpath=//*[@id="app-content"]/div/div/div/div[2]/div/div[3]/div/div[2]/button[2]'
                );

                await sleep(6000);
                this.metamaskLoginPage = this.context?.pages()[2];
                await this.metamaskLoginPage?.click(
                    'xpath=//*[@id="app-content"]/div/div/div/div/div[3]/button[2]'
                );
                await sleep(10000);
                ...
                for (let i = 0; i < 2; i++) {
                    try {
                        await this.page?.click(
                            `xpath=//*[@id="modal-root"]/aside/div[2]/section/div/div[1]/button`,
                            { timeout: 10000 }
                        );
                    } catch {}
                }
                ...
                if (currentUrl !== "https://www.abs.xyz/wallet") {
                    logger.error(
                        `${this.accountIndex} | Failed to reach wallet page, current URL: ${currentUrl}`
                    );
                    continue;
                }
                logger.success(
                    `${this.accountIndex} | Successfully logged in to abs`
                );
                return true;
            } catch (error) {
                logger.error(
