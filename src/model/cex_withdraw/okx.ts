...
export class OKXWithdraw {
    ...
    async withdraw(): Promise<boolean> {
        try {
            logger.info(
                `${this.accountIndex} | ${this.address} | Starting OKX withdrawal`
            );
            ...
            logger.success(
                `${this.accountIndex} | ${this.address} | Successfully withdrew from OKX! Waiting for deposit...`
            );
            ...
            while (attempts < maxAttempts) {
                ...
                if (attempts % ATTEMPTS_PER_LOG === 0) {
                    ...
                    logger.info(
                        `${this.accountIndex} | ${
                            this.address
                        } | Waiting for deposit... ${minutesWaited.toFixed(
                            1
                        )} minutes`
                    );
                }
                ...
                await new Promise((resolve) =>
                    setTimeout(resolve, SECONDS_PER_ATTEMPT * 1000)
                );
            }
            ...
            logger.error(
                `${this.accountIndex} | ${this.address} | Error withdrawing from OKX: ${error}`
            );
            return false;
        }
    }
