...
export class RelayBridge {
    ...
    private async _calculateBridgeAmount(): Promise<number> {
        try {
            // Get current balance
            ...
            logger.info(
                `${this.accountIndex} | ${this.wallet.address} | Balance: ${ethBalance} ETH, Bridging fixed amount: ${amountToBridge} ETH`
            );

            return amountToBridge;
        } catch (error) {
            logger.error(
                `${this.accountIndex} | ${this.wallet.address} | Error calculating bridge amount: ${error}`
            );
            return 0;
        }
    }
        ...
        try {
            ...
            if (receipt && receipt.status === 1) {
                ...
                const pauseTime = randomInt(
                    this.config.settings.pause_after_bridge[0],
                    this.config.settings.pause_after_bridge[1]
                );
                logger.info(
                    `${this.accountIndex} | ${
                        this.wallet.address
                    } | Waiting ${pauseTime.toString()} seconds for tokens to appear in Abstract wallet...`
                );
            ...
            logger.error(
                `${this.accountIndex} | ${this.wallet.address} | Error bridge: ${error}`
            );
            return false;
        }
    }
