...
export class MainBridge {
    ...
    async bridge(): Promise<boolean> {
        try {
            ...
            const amountWithFees =
                (amountWei * BigInt(1000048724269594n)) /
            ...
            const maxFee =
                (baseFee * BigInt(110)) / BigInt(100) + maxPriorityFee;

            // Build transaction
            const nonce = await this.wallet.getNonce();
            const transaction =
                await contract.requestL2TransactionDirect.populateTransaction(
                    request,
                    {
            ...
            if (receipt && receipt.status === 1) {
                logger.success(
                    `${this.wallet.address} | Bridge transaction confirmed: ${tx.hash}`
                );
                return true;
            } else {
                logger.error(
     ...
     */
    private getRandomGasLimit(): number {
